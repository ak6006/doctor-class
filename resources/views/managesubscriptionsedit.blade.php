...
<div class="container-fluid my-5 text-center text-blue">
            <h2><span class="pb-1 border-buttom" > تعديل الاشتراك </span></h2>
...
<a target="_new" href="companyview?coid=<?=$companie->id?>" class="btn bg-blue text-white" title=""><?=$companie->name;?> </a>
...
<?php } ?>
                    ...
                    <tfoot>
                        <tr>
                            <td colspan="7">
							<a target="_self" href="managesubscriptionsstop?subid=<?=$subscription->id?>" class="btn btn-danger text-right" title="إيقاف الباقة">إيقاف الباقة </a>
							<a target="_new" href="subscriptions?maxid=<?=$subscription->id?>" class="btn btn-success text-right" title="تجديد الباقة مدة اخرى">تجديد الباقة مدة اخرى  </a>
							</td>
			   ...
			   <a href="managesubscriptionsedit?subid=<?=$subscription->id?>&activate=1" class="btn btn-success" title="Show">تفعيل</a>
			   </td>
<?php }else { ?>
<td class="text-center"> 
			   <a href="managesubscriptionsedit?subid=<?=$subscription->id?>&activate=0" class="btn btn-success" title="Show">الغاء التفعيل</a>
			   </td>
<?php } ?>	 
			   ...
			   <td class="text-center">      
               <a href="managesubscriptionsedit?subid=<?=$subscription->id?>&pay=1" class="btn btn-danger" title="Delete">تأكيد الدفع</a></td>
			 <?php }else { ?>
 <td class="text-center">  <a href="managesubscriptionsedit?subid=<?=$subscription->id?>&pay=0" class="btn btn-danger" title="Delete">الغاء الدفع</a></td>
<?php } ?>
